                    ...
                    <tr key={transfer.id} className="border-t border-[var(--line)] align-middle hover:bg-[var(--panel-strong)]/40">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex flex-wrap justify-end gap-1.5">
                          {transfer.status === "REQUESTED" ? <form action={approveStockTransferAction}><input type="hidden" name="id" value={transfer.id} /><button className="rounded-lg border border-[var(--line)] px-2.5 py-1 text-xs font-semibold text-[var(--ink)] hover:border-[var(--accent)]/50">Approve</button></form> : null}
                          {transfer.status === "APPROVED" ? <form action={dispatchStockTransferAction}><input type="hidden" name="id" value={transfer.id} /><button className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-2.5 py-1 text-xs font-semibold text-amber-700">Dispatch</button></form> : null}
                          {transfer.status === "DISPATCHED" ? <form action={receiveStockTransferAction}><input type="hidden" name="id" value={transfer.id} /><button className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-xs font-semibold text-emerald-700">Receive</button></form> : null}
                          {transfer.status === "REQUESTED" || transfer.status === "APPROVED" ? <form action={cancelStockTransferAction}><input type="hidden" name="id" value={transfer.id} /><button className="rounded-lg border border-red-500/20 px-2.5 py-1 text-xs font-semibold text-red-600">Cancel</button></form> : null}
                        </div>
                      </td>
                    </tr>
                  );
