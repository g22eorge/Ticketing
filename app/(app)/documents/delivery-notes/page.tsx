    ...
    <section className="space-y-4">
      <div className="panel-shadow overflow-hidden rounded-2xl border border-[var(--line)] bg-gradient-to-r from-sky-100 via-white to-orange-100 p-4 text-slate-950 sm:p-6 dark:from-slate-900 dark:via-slate-950 dark:to-slate-900 dark:text-[var(--ink)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-700 dark:text-[var(--ink-muted)]">Proof of delivery for invoice-based repair/document workflows. POS quick sales do not generate delivery notes.</p>
          </div>
          <Link href="/documents/invoices" className="btn-premium rounded-full px-4 py-2 text-sm text-white">Open Invoices</Link>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <div className="panel-shadow rounded-xl border border-[var(--line)] bg-[var(--panel)] p-4">
          ...
          <p className="mt-2 text-2xl font-semibold text-[var(--ink)]">{uniqueSources}</p>
        </div>
      </div>
          ...
          <tbody>
            {notes.map((n) => (
              <tr key={n.id} className="border-t border-[var(--line)]">
                ...
                <td className="hidden px-3 py-2 text-[var(--ink-muted)] lg:table-cell">{n.invoice?.job.client.fullName ?? n.sale?.client?.fullName ?? "-"}</td>
