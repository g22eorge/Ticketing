...
export default async function SalesPage({
  ...
  const [leads, quotations, leadCounts] = await Promise.all([
    activeTab === "leads"
    ...
    <div className="space-y-4">
      <div className="panel-shadow flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[var(--line)] bg-[var(--panel)] px-4 py-3">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-[var(--ink-muted)]">Manage leads and quotations</p>
        </div>
        ...
        {activeTab === "leads" ? (
          <div>
            <div className="flex flex-wrap items-center gap-2 border-b border-[var(--line)] px-4 py-2.5">
              ...
              >
                <span className={!statusFilter ? "text-white font-bold" : "font-bold text-[var(--ink)]"}>{totalLeads}</span> all
              </Link>
              {(Object.keys(LEAD_STATUS_LABELS) as LeadStatus[]).map((s) => (
                <Link
                  key={s}
                  href={`/sales?tab=leads&status=${s}`}
                  className={`rounded-full border px-3 py-1 text-[11px] font-semibold transition-colors ${
                    statusFilter === s
                      ? "border-[var(--accent)] bg-[var(--accent)] text-white"
                      : "border-[var(--line)] bg-[var(--panel-strong)] text-[var(--ink-muted)] hover:border-[var(--accent)]/40"
                ...
                >
                  <span className={statusFilter === s ? "text-white font-bold" : "font-bold text-[var(--ink)]"}>{statusCounts[s] ?? 0}</span> {LEAD_STATUS_LABELS[s]}
                </Link>
              ))}
            </div>
            ...
            {showNewLead && can.createLeads(user) ? (
              <div className="border-b border-[var(--line)] px-4 py-3">
                {filters.createError ? (
                  <p className="mb-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-600">{filters.createError}</p>
                ) : null}
                <form action={createLeadAction} noValidate className="space-y-2">
                  <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--ink-muted)]">New Lead</p>
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    <input name="fullName" required placeholder="Full name *" className="rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-sm outline-none focus:border-[var(--accent)]/50 focus:ring-2 focus:ring-[var(--accent)]/15" />
                    <input name="phone" required placeholder="Phone *" className="rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-sm outline-none focus:border-[var(--accent)]/50 focus:ring-2 focus:ring-[var(--accent)]/15" />
                    <input name="email" placeholder="Email" className="rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-sm outline-none focus:border-[var(--accent)]/50 focus:ring-2 focus:ring-[var(--accent)]/15" />
                    <input name="organization" placeholder="Organization" className="rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-sm outline-none focus:border-[var(--accent)]/50 focus:ring-2 focus:ring-[var(--accent)]/15" />
                    <input name="interest" placeholder="Interest / product" className="rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-sm outline-none focus:border-[var(--accent)]/50 focus:ring-2 focus:ring-[var(--accent)]/15" />
                    <input name="estimatedValue" type="number" placeholder={`Est. value (${currency})`} className="rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-sm outline-none focus:border-[var(--accent)]/50 focus:ring-2 focus:ring-[var(--accent)]/15" />
                    <select name="source" className="rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-sm outline-none focus:border-[var(--accent)]/50">
                    ...
                    <textarea name="notes" placeholder="Notes" rows={2} className="rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-sm outline-none focus:border-[var(--accent)]/50 focus:ring-2 focus:ring-[var(--accent)]/15 sm:col-span-2" />
                  </div>
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    <Link href="/sales?tab=leads" className="text-xs font-medium text-[var(--ink-muted)] underline-offset-2 hover:underline">Cancel</Link>
                  </div>
                </form>
              </div>
            ) : null}
            ...
            {leads.length === 0 ? (
              <div className="flex flex-col items-center gap-2 px-6 py-14 text-center">
                ...
                {statusFilter ? <Link href="/sales?tab=leads" className="text-xs text-[var(--accent)] hover:underline">Clear filter</Link> : null}
              </div>
            ) : (
                    ...
                    {leads.map((lead) => (
                      <tr key={lead.id} className="transition-colors hover:bg-[var(--panel-strong)]/40">
                        <td className="px-4 py-3 font-medium text-[var(--ink)]">
                          <Link href={`/sales/leads/${lead.id}`} className="hover:text-[var(--accent)] hover:underline">
                            {lead.fullName}
                          </Link>
                          {lead.organization ? (
                            <span className="ml-1.5 text-[11px] text-[var(--ink-muted)]">{lead.organization}</span>
                          ) : null}
                        </td>
                        <td className="px-4 py-3 text-[var(--ink-muted)]">{lead.phone}</td>
                        <td className="px-4 py-3 text-[var(--ink-muted)]">{lead.source.replace("_", " ")}</td>
                        ...
                        <td className="px-4 py-3 text-[var(--ink-muted)]">
                          {lead.estimatedValue != null ? formatMoney(lead.estimatedValue, currency) : <span className="opacity-40">—</span>}
                        </td>
                        <td className="px-4 py-3 text-[var(--ink-muted)]">
                          {lead.followUpAt ? formatEATDate(lead.followUpAt) : <span className="opacity-40">—</span>}
                        </td>
                      ...
                      return (
                        <tr key={q.id} className="transition-colors hover:bg-[var(--panel-strong)]/40">
                          <td className="px-4 py-3 font-mono text-[12px] font-semibold text-[var(--ink)]">
                            <Link href={`/sales/quotations/${q.id}`} className="hover:text-[var(--accent)] hover:underline">
                              {q.quoteNumber}
                            </Link>
                          </td>
                          ...
                          <td className="px-4 py-3 text-[var(--ink-muted)]">
                            {q.validUntil ? formatEATDate(q.validUntil) : <span className="opacity-40">—</span>}
                          </td>
                          <td className="px-4 py-3 text-right">
                            <Link
