      ...
      {BOARD_STATUSES.map((status) => {
        ...
        return (
          <div key={status} className="flex w-72 shrink-0 flex-col">
            {/* Column header */}
            <div className="mb-3 flex items-center gap-2.5 rounded-xl bg-white px-4 py-3 shadow-sm ring-1 ring-stone-200">
              <span
                className="h-2.5 w-2.5 rounded-full"
                style={{ backgroundColor: meta.accent }}
                aria-hidden="true"
              />
              <span className="flex-1 text-xs font-semibold tracking-wide text-stone-700">
                {meta.label}
              </span>
              <span
            ...
            {/* Tickets */}
            <div className="flex min-h-[120px] flex-col gap-3 rounded-xl bg-stone-100/60 p-2.5 ring-1 ring-stone-200/50">
              {colJobs.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-2 py-8 text-center">
                  <svg className="h-8 w-8 text-stone-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                    <rect x="3" y="3" width="18" height="18" rx="2" />
                    <line x1="12" y1="8" x2="12" y2="12" />
                    <line x1="12" y1="16" x2="12.01" y2="16" />
                  </svg>
                  <span className="text-sm text-stone-400">No tickets</span>
                </div>
              ) : (
                colJobs.map((job) => {
                  ...
                  return (
                    <Link
                      key={job.id}
                      href={`/jobs/${job.id}`}
                      className="group relative overflow-hidden rounded-xl bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md ring-1 ring-stone-200/70 hover:ring-amber-200/60"
                    >
                      {/* Left accent bar */}
                      <span
                      ...
                      <div className="relative ml-2 space-y-2.5">
                        ...
                        <div className="flex items-center gap-2 text-stone-500">
                          ...
                          <span className="truncate text-sm font-medium text-stone-700">{device}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-1 text-xs text-stone-400">
                          ...
                          {assignedName ? (
                            <span className="shrink-0 rounded-md bg-stone-50 px-2 py-0.5 text-xs font-medium text-stone-500">
                              {assignedName}
                            </span>
                          ) : null}
                        </div>
                        ...
                        {(job.clientBill !== undefined || job.externalTechBill !== undefined) && (
                          <div className="flex items-center gap-2 pt-1 text-xs text-stone-400">
                            {job.clientBill !== undefined && (
                              <span className="rounded bg-stone-50 px-2 py-0.5 text-stone-500">
