...
export function TicketDetailView({ job, auditLogs, baseCurrency, returnTo }: Props) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-4 pb-20 md:p-6">
      {/* Back */}
      <Link href={returnTo} className="inline-flex items-center gap-1 text-sm font-medium text-stone-500 transition hover:text-stone-900">
      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-2">
            <p className="text-sm font-medium text-stone-500">Ticket {job.jobNumber}</p>
            <h1 className="text-xl font-semibold tracking-tight text-stone-900">
              {job.brand || job.deviceType || "Device"}
              {job.model ? ` · ${job.model}` : ""}
            </h1>
            <p className="text-sm text-stone-500">Received {fmtDate(job.receivedAt)}</p>
          </div>
          <span
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          {job.client && (
            <Section title="Client">
              <p className="text-base font-semibold text-stone-900">{job.client.fullName}</p>
              ...
              <div className="flex flex-wrap gap-4 text-sm">
                {job.deviceType && (
                  <div>
                    ...
                    <p className="font-medium text-stone-900">{job.deviceType}</p>
                  </div>
                )}
                {job.serialOrImei && (
                  <div>
                    <p className="text-xs text-stone-500">Serial Number</p>
                    <p className="font-mono text-sm text-stone-700">{job.serialOrImei}</p>
                  </div>
                )}
              </div>
            </Section>
          ...
          {job.issueDescription && (
            <Section title="Issue Description">
              <p className="whitespace-pre-wrap text-sm leading-relaxed text-stone-700">{job.issueDescription}</p>
            </Section>
          )}
          ...
          {job.diagnosisNotes && (
            <Section title="Diagnosis">
              <p className="whitespace-pre-wrap text-sm leading-relaxed text-stone-700">{job.diagnosisNotes}</p>
            </Section>
          )}
          ...
          <Section title="Timeline">
            <div className="space-y-3">
              {auditLogs.map((a) => (
                <div key={a.id} className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm text-stone-700">{a.action}</p>
              ...
              {auditLogs.length === 0 && <p className="text-sm text-stone-500">No activity recorded.</p>}
            </div>
          </Section>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
            <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-stone-500">Status</h3>
            <div className="space-y-3">
              {job.assignedTo && (
                <div className="flex items-center gap-2 text-sm">
                  <svg className="h-4 w-4 text-stone-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>
                  <span className="text-stone-600">Assigned to <span className="font-medium text-stone-900">{job.assignedTo.name}</span></span>
                </div>
              )}
              {job.clientBill != null && (
                <div className="flex items-center gap-2 text-sm">
          ...
          {nextStatuses.length > 0 && (
            <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
              <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-stone-500">Update Status</h3>
              <input
                type="text"
                placeholder="Add a note (optional)"
                value={note}
                onChange={(e) => setNote(e.target.value)}
                className="mb-3 w-full rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm outline-none transition focus:border-stone-400 focus:ring-1 focus:ring-stone-400"
              />
              <div className="flex flex-wrap gap-2">
                {nextStatuses.map((s) => (
                  <button
                    ...
                    className="rounded-lg bg-stone-900 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-stone-800 disabled:opacity-40"
                  >
          ...
          {/* Documents */}
          <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
            <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-stone-500">Documents</h3>
            <div className="space-y-2">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-stone-500">{title}</h3>
      {children}
    </div>
  );
}
...
function DocLink({ href, label, icon, exists }: { href: string; label: string; icon: string; exists: boolean }) {
  ...
  return (
    <Link
      href={href}
      className="flex items-center gap-3 rounded-xl border border-stone-200 px-4 py-3 text-sm transition hover:border-stone-300 hover:bg-stone-50"
    >
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-stone-100">
        <svg className="h-4 w-4 text-stone-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d={icons[icon] ?? icons.job} />
        </svg>
      </div>
      <span className="font-medium text-stone-900">{label}</span>
      <span className="ml-auto text-xs text-stone-400">{exists ? "View" : "Generate"}</span>
    </Link>
  );
}
