...
export const can = {
  ...
  viewClientInfo: (user: PermissionUser) =>
    ...
    && (["ADMIN", "MANAGER", "TECH_MANAGER", "FINANCE", "SALES", "OPS", "FRONT_DESK", "SALES_MANAGER", "SALES_CORPORATE", "SALES_RETAIL"].includes(user.role)
    || hasExtraPermission(user, "can_intake")),

  viewFinancials: (user: PermissionUser) =>
    user.role !== "TECHNICIAN_EXTERNAL"
    && ([...OPS_ROLES, ...FINANCE_ROLES, "FRONT_DESK", "SALES_MANAGER"].includes(user.role)
  ...
  createJob: (user: PermissionUser) =>
    [...SALES_ROLES, "FRONT_DESK", "TECH_MANAGER"].includes(user.role)
    || hasExtraPermission(user, "can_intake"),

  editDiagnosis: (user: PermissionUser) =>
    [...TECH_ROLES, "TECHNICIAN_EXTERNAL"].includes(user.role)
    || hasExtraPermission(user, "can_run_internal_repairs"),

  assignJobs: (user: PermissionUser) =>
    ["ADMIN", "MANAGER", "OPS", "TECH_MANAGER"].includes(user.role)
  ...
  generateJobCards: (user: PermissionUser) =>
    user.role !== "TECHNICIAN_EXTERNAL"
    && (["ADMIN", "MANAGER", "TECH_MANAGER", "OPS", "FRONT_DESK", "TECHNICIAN_INTERNAL"].includes(user.role)
    || hasExtraPermission(user, "can_generate_job_cards")),

  viewApprovedCost: (user: PermissionUser) =>
    user.role !== "TECHNICIAN_EXTERNAL"
    && (["ADMIN", "MANAGER", "TECH_MANAGER", "FINANCE", "OPS", "FRONT_DESK", "SALES_MANAGER"].includes(user.role)
    || hasExtraPermission(user, "can_view_approved_cost")),

  reviewExternalBills: (user: PermissionUser) =>
    user.role !== "TECHNICIAN_EXTERNAL"
    && (["ADMIN", "MANAGER", "TECH_MANAGER", "FINANCE"].includes(user.role)
    || hasExtraPermission(user, "can_review_external_bills")),

  // Finance
  viewAccountsSummary: (user: PermissionUser) =>
    user.role !== "TECHNICIAN_EXTERNAL"
    && (["ADMIN", "MANAGER", "FINANCE", "OPS", "SALES_MANAGER"].includes(user.role)
    || hasExtraPermission(user, "can_view_accounts_summary")),
  ...
  runFinancialReports: (user: PermissionUser) =>
    ["ADMIN", "MANAGER", "SALES_MANAGER", "FINANCE"].includes(user.role)
  ...
  approveQuotations: (user: PermissionUser) =>
    ["ADMIN", "MANAGER", "SALES_MANAGER", "TECH_MANAGER"].includes(user.role)
    || hasExtraPermission(user, "can_approve_quotations"),

  overrideDiscount: (user: PermissionUser) =>
    ["ADMIN", "MANAGER", "SALES_MANAGER"].includes(user.role)
  ...
  openPosSession: (user: PermissionUser) =>
    ["ADMIN", "MANAGER", "OPS", "FRONT_DESK", "SALES", "SALES_MANAGER", "SALES_RETAIL", "SALES_POS"].includes(user.role)
  ...
  manageSalesTargets: (user: PermissionUser) =>
    ["ADMIN", "MANAGER", "SALES", "SALES_MANAGER"].includes(user.role)
    || hasExtraPermission(user, "can_manage_targets"),

  // Field work
  manageFieldVisits: (user: PermissionUser) =>
    ["ADMIN", "MANAGER", "OPS", "TECH_MANAGER", "TECH_FIELD"].includes(user.role)
